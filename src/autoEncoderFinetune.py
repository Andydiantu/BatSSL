...
# test_loader  = getDataLoaderSplit(32,False,False,1.0)

# train_loader, test_loader = genTrainTestLoader(batchSize = 32, dataAug = True, dataCount = args.numData, testAmount = 0.25)
...
for epoch in range(EPOCHS):
    ...
    if eval_acc > best_acc:
        best_acc = eval_acc
        torch.save(DSmodel.state_dict(), 'src/models/bestAutoEncoderFinetune.pth')



DSmodel.load_state_dict(torch.load('src/models/bestAutoEncoderFinetune.pth')) 
trueLabels, predictLabels = evalModelNew(DSmodel, test_loader,classes)
save_plots(train_acc, train_loss, valid_acc, valid_loss, f'Autoencoder{(args.datasetPath)[13:]}data_{args.epochs_Finetune}epochs')
