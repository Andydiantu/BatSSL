...
for epoch in range(EPOCHS):
    ...
    if eval_acc > best_acc:
        best_acc = eval_acc
        torch.save(DSmodel.state_dict(), 'src/models/bestBaselineFinetune.pth')
    
DSmodel.load_state_dict(torch.load('src/models/bestBaselineFinetune.pth')) 
trueLabels, predictLabels = evalModelNew(DSmodel, test_loader,classes)
save_plots(train_acc, train_loss, valid_acc, valid_loss, f'BaselineFinetune_{(args.datasetPath)[13:]}_data_{args.epochs_Finetune}epochs')
